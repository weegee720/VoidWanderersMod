-----------------------------------------------------------------------------------------
-- Initialize global faction lists
-----------------------------------------------------------------------------------------
function CF_InitScenesData(activity)
	-- Define planets
	CF_Planet = {}
	CF_PlanetName = {}
	CF_PlanetGlow = {}
	CF_PlanetGlowModule = {}

	-- Define station
	local id = "TradeStar"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "FreeTrade TradeStar"
	CF_PlanetGlow[id] = "Station"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"
	
	-- Define basic planet
	local id = "Planet"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Planet"
	CF_PlanetGlow[id] = "Planet"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"

	-- Define moon
	local id = "Moon"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Moon"
	CF_PlanetGlow[id] = "Moon"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"

	local id = "Moon 2"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Moon 2"
	CF_PlanetGlow[id] = "Moon"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"

	local id = "Moon 3"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Moon 3"
	CF_PlanetGlow[id] = "Moon"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"

	local id = "Moon 4"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Moon 4"
	CF_PlanetGlow[id] = "Moon"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"

	local id = "Moon 5"
	CF_Planet[#CF_Planet + 1] = id
	CF_PlanetName[id] = "Moon 5"
	CF_PlanetGlow[id] = "Moon"
	CF_PlanetGlowModule[id] = "VoidWanderers.rte"
	
	-- Define locations
	CF_Location = {}
	CF_LocationName = {}
	CF_LocationPos = {}
	CF_LocationSecurity = {}
	CF_LocationScene = {}
	CF_LocationPlanet = {}
	
	local id = "TradeStar Pier #792"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "TradeStar Pier #792"
	CF_LocationPos[id] = nil
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "TradeStar"

	local id = "Test 1"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "Test 1"
	CF_LocationPos[id] = Vector( -25, 25)
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "Planet"

	local id = "Test 2"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "Test 2"
	CF_LocationPos[id] = Vector( 25, 25)
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "Planet"

	local id = "Test 3"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "Test 3"
	CF_LocationPos[id] = Vector( 25, 25)
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "Moon"

	local id = "Test 4"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "Test 4"
	CF_LocationPos[id] = Vector( 25, 25)
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "Moon"
	
	local id = "Test 5"
	CF_Location[#CF_Location + 1] = id
	CF_LocationName[id] = "Test 5"
	CF_LocationPos[id] = Vector( 25, 25)
	CF_LocationSecurity[id] = 0
	CF_LocationScene[id] = "TradeStar Pier #792"
	CF_LocationPlanet[id] = "Moon"
end